...
func DeregisteredInitiated(state *fsm.State, event fsm.EventType, args fsm.ArgsType) {
	...
	case fsm.EntryEvent:
		...
		gmmMessage := args[ArgNASMessage].(*nas.GmmMessage)
		if gmmMessage != nil {
			amfUe.GmmLog.Debugln("EntryEvent at GMM State[DeregisteredInitiated] - UE triggered Deregisration")
			accessType := args[ArgAccessType].(models.AccessType)
			...
			amfUe.GmmLog.Debugln("EntryEvent at GMM State[DeregisteredInitiated] - Network triggered Deregisration")
		}
	case GmmMessageEvent:
		amfUe := args[ArgAmfUe].(*context.AmfUe)
		gmmMessage := args[ArgNASMessage].(*nas.GmmMessage)
